...
def get_connection():
    ...
    return pymysql.connect(
        host="localhost",
        user="root",            # 네 실제 MariaDB 계정
        password="1234",        # 네 MariaDB root 비밀번호
        database="a123456",     # 네가 만든 DB 이름
        charset="utf8mb4",
        cursorclass=pymysql.cursors.DictCursor
    )

def insert_html_document(url, title, raw_text):
...
#    -----------------------------------------------------------
    ...
    try:
        with conn.cursor() as cur:
            sql = """
            INSERT INTO documents (source_type, url, title, raw_text, clean_text)
            VALUES ('html', %s, %s, %s, NULL)
            """
            cur.execute(sql, (url, title, raw_text))
            conn.commit()
...
def insert_file_document(source_type, url, file_path, clean_text, ref_html_id=None, raw_text=None, title=None):
    #    -----------------------------------------------------------
    #     [수정사항]
    #     - 인자로 받은 raw_text(추출된 텍스트)를 DB의 raw_text 컬럼에 저장
    #     - clean_text 컬럼은 추후 처리를 위해 NULL로 비워둠
    ...
    try:
        with conn.cursor() as cur:
            ...
            (source_type, url, ref_html_id, title, raw_text, clean_text, file_path)
            VALUES (%s, %s, %s, %s, %s, NULL, %s)
            """
            cur.execute(sql, (
                source_type, url, ref_html_id, title,
                raw_text, file_path
            ))
            conn.commit()
            return cur.lastrowid
